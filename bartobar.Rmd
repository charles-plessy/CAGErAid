...
ce <-
  CAGEexp( metadata = list(genomeName = BSgenomeToUse)
         , colData  = DataFrame( inputFiles     = pathsToInputFiles[1:8]
                               , sampleLabels   = sampleLabels[1:8]
                               , inputFilesType = "bamPairedEnd"
                               , row.names      = sampleLabels[1:8]))

ce <- getCTSS(ce, useMulticore = TRUE, nrCores = 8, correctSystematicG=FALSE, removeFirstG=FALSE)
colData(ce)
```

Polish the sample data and fixing sample names
